...
# _v0은 버전으로 치지 않는다(빈 값) — 로컬 초안 보관용. 업로드 파이프라인 전체가
# 무시하되, _v 꼴이라 자동 넘버링(새 작품 오인)도 타지 않는다.
verof() {   # verof 0819_33_v2.png -> "33 2"
  printf '%s\n' "$1" | sed -n "s/^\([0-9][0-9]*_\)\{0,1\}\([0-9][0-9]\)_v\([1-9][0-9]*\)\.png\$/\2 \3/p"
}
...
vcheck 0819_33_v0.png     ""      # v0 = 로컬 초안, 업로드 안 함
