...
try {
  ...
  if (manual) {
    ...
    //
    // 농담 정규화: 어떤 회색으로 그렸든 결과 톤이 같도록, 잉크 픽셀(≤230)의
    // **중앙값**을 검정으로 당겨 늘린 뒤 위 매핑을 적용한다 — 기준이 극단값(가장
    // 진한 몇 픽셀)이면 붓 가장자리만 진한 그림에서 몸통이 여전히 연하게 남는다
    // (33·36에서 실제로 겪었다). 중앙값 기준이면 실루엣 본체가 정확히 102(#666)에
    // 안착하고, 중앙값보다 진한 절반은 102로 눌린다 — 검정으로 그린 그림은 원래
    // 평평해서 차이가 없고, 밝은 쪽 농담(질감)은 비례로 보존된다.
    const histo = new Uint32Array(256);
    for (let p = 0; p < N; p++) histo[lum[p]]++;
    let inkN = 0;
    for (let i = 0; i <= 230; i++) inkN += histo[i];
    let cnt = 0, ink = 0;
    for (let i = 0; i <= 230; i++) { cnt += histo[i]; if (cnt >= inkN / 2) { ink = i; break; } }
    if (!inkN) ink = 0;   // 사실상 빈 그림 — 정규화 안 함
